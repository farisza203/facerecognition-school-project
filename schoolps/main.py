...
facerecog_Mainbutton= ImageTk.PhotoImage((Image.open('./Asset/facerecognitionmain.png')).resize((240,100), Image.ANTIALIAS))
facerecog_Mainbutton_change= ImageTk.PhotoImage((Image.open('./Asset/facerecognitionmain_change.png')).resize((240,100), Image.ANTIALIAS))
facedetect_Mainbutton= ImageTk.PhotoImage((Image.open('./Asset/facedetectmain.png')).resize((240,100), Image.ANTIALIAS))
facedetect_Mainbutton_change=ImageTk.PhotoImage((Image.open('./Asset/facedetectmain_change.png')).resize((240,100), Image.ANTIALIAS))
...
def save_file():
        ...
        if Name != "":
            ...
            if not os.path.exists(newpath):
                ...
                print("You have 1 picture in floder now")
                print("In floder have = {} ".format(os.listdir(Path)))
                print("from Path = {} \n".format(Path))
            else :
...
def facedetect():
    ...
    def show_frames(image_label, video_capture):
        ...
        #กำลังขยายที่ดีที่สุดคือ 1/2 หรือ 1/3
        small_frame = cv2.resize(rgb, (0, 0), fx=1/3, fy=1/3)
        num_test =3
        try:
            ...
            for ( location, name,percent) in zip( face_locations, face_name,face_percent):
                if name == 'Unknown':
                    ...
                    color_Match=[0,255,0]
                cv2.rectangle(frame, (location[3]*num_test, location[0]*num_test), (location[1]*num_test,location[2]*num_test), color_rectangle, 2)
                cv2.putText(frame, name, (location[3]*num_test, location[0]*num_test), cv2.FONT_HERSHEY_COMPLEX,1, (255, 255, 255), 2)
                cv2.putText(frame,'Matches '+str(percent)+' %',(location[3]*num_test,(location[2]*num_test)+16),cv2.FONT_HERSHEY_COMPLEX,0.5, color_Match, 1)
        except:
            face_locations = face_recognition.face_locations(small_frame)
            for (TOP,RIGHT,BOTTOM,LEFT) in face_locations:
                ...
                color_rectangle= [46,2,209]
                color_Match=[0, 0, 255]
                cv2.rectangle(frame, (LEFT*num_test, TOP*num_test), (RIGHT*num_test,BOTTOM*num_test), color_rectangle, 2)
                cv2.putText(frame, name, (LEFT*num_test, TOP*num_test), cv2.FONT_HERSHEY_COMPLEX,1, (255, 255, 255), 2)
                cv2.putText(frame,'Matches '+'0'+' %',(LEFT*num_test,(BOTTOM*num_test)+16),cv2.FONT_HERSHEY_COMPLEX,0.5, color_Match, 1)
        cv2.waitKey(0)
        img = ImageTk.PhotoImage(Image.fromarray(cv2.cvtColor(cv2.resize(frame,(0,0),fx=1.35,fy=1.35), cv2.COLOR_BGR2RGB)))
        image_label.configure(image=img)
        image_label._image_cache = img 
